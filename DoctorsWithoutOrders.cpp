...
bool canAllPatientsBeSeenRec(const HashMap<string, int>& doctors,
                             const HashMap<string, int>& patients,
                             HashMap<string, HashSet<string>>& schedule,
                             HashMap<string, HashSet<string>>& chosenPatients){
    HashMap<string, int> backUpDoctors = doctors;
    HashMap<string, int> backUpPatients = patients;


    if (backUpPatients.isEmpty()){
        schedule = chosenPatients;
        return true;
    }

    string patient = backUpPatients.front();

    for (string doctor: doctors) {
        if (backUpPatients[patient] <= backUpDoctors[doctor]) {

            //Try out each doctor
            backUpDoctors[doctor] -= backUpPatients[patient];
            int patientHours = backUpPatients[patient];
            backUpPatients.remove(patient);
            chosenPatients[doctor] += patient;

            if (canAllPatientsBeSeenRec(backUpDoctors, backUpPatients, schedule, chosenPatients)) {
                schedule = chosenPatients;
                return true;
            }

            backUpDoctors[doctor] += patientHours;
            backUpPatients.add(patient, patientHours);
            chosenPatients[doctor] -= patient;
        }
    }
...
bool canAllPatientsBeSeen(const HashMap<string, int>& doctors,
    ...
    return canAllPatientsBeSeenRec(doctors, patients, schedule, chosenPatients);
...
ADD_TEST("Returns true if there are no patients") {
    ...
    HashMap<string, int> patients = {
        { },  // Needs more time than any one doctor can provide
    };
